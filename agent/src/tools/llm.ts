...
/**
 * Which fixture a dry-run call wants. The repair prompt is the only one carrying
 * <validation_errors>, and every generator/repair prompt names its target file, so the
 * fixture set can be addressed without changing the documented call signatures.
 *
...
function resolveFixture(user: string): { label: string; content: string } {
  ...
  const generatorMatch = Object.keys(MOCK_FILES).find((path) => user.includes(path));
  if (isRepair && generatorMatch !== undefined) {
    throw new Error(
      `[dry-run] ${label} fixture lookup failed: "${generatorMatch}" has a generator fixture but ` +
  ...
  throw new Error(
    `[dry-run] ${label} fixture lookup failed: the prompt names none of the known fixture ` +
      `files (${Object.keys(table).join(', ')}). Fixtures are addressed by matching the file ` +
      `path in the prompt text, so a prompt that no longer names its target file breaks this ` +
      `lookup. No fallback is served — re-run the dry run after any prompt change.`,
  );
}
